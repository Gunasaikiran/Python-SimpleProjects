...
#Here we will create geometry or a so-called layout for the GUI of the calculator by using Tkinter.
root = Tk()
...
# i.e buttons
calc.grid()
# Now we will create a class in which we will create all the functions of the scientific calculator so that they can be called and perform easily.
class Calc():
    def __init__(self):
        self.total = 0
        self.current = ''
...
added_value = Calc()
#The below code will create a display in the GUI of the calculator by passing the font style, font size, background color, foreground color as an argument inside the entry function. 
...
#Here we will create a number pad for the calculator. 
...
# Now at last we will create a menubar of the calculator GUI. 
